...
export const CifraClubSidebar: React.FC<CifraClubSidebarProps> = ({
  ...
  return (
    <aside className="w-full lg:w-64 shrink-0 bg-[#181818] border-r border-zinc-800 flex flex-col select-none text-zinc-300">
      ...
      <div className="p-3.5 border-b border-zinc-800 flex items-center justify-between text-zinc-400 relative">
        <button
          onClick={onBack}
          title="Voltar"
          className="p-2 sm:p-2.5 rounded-xl hover:bg-zinc-800 hover:text-white transition-colors active:scale-95"
        >
          <ChevronLeft size={20} />
        </button>
        ...
        <div className="flex items-center gap-1 sm:gap-1.5">
          <button
            ...
            title="Imprimir"
            className="p-2 sm:p-2.5 rounded-xl hover:bg-zinc-800 hover:text-white transition-colors"
          >
            <Printer size={18} />
          </button>
          <button
            ...
            className={`p-2 sm:p-2.5 rounded-xl hover:bg-zinc-800 transition-colors ${
          ...
          <button
            ...
            className="p-2 sm:p-2.5 rounded-xl hover:bg-zinc-800 hover:text-orange-500 transition-colors relative"
          >
        ...
        {copiedLink && (
          <div className="absolute -bottom-8 right-2 bg-zinc-900 border border-orange-500 text-orange-400 text-xs px-3 py-1 rounded-md shadow-lg animate-fadeIn font-semibold">
            Link copiado!
          </div>
        )}
      </div>
      ...
      {/* Lista de Controles */}
      <div className="flex-1 overflow-y-auto divide-y divide-zinc-800/80 text-xs sm:text-[13px]">

        {/* 1. Rolagem */}
        <div className="p-3.5 hover:bg-zinc-850/50 transition-colors">
          <div
          ...
          >
            <div className="flex items-center gap-2.5">
              <div className="w-5 h-5 flex items-center justify-center text-zinc-400">
                {isPlaying ? <Pause size={17} className="text-orange-500" /> : <Play size={17} />}
              </div>
            ...
            <div className="flex items-center gap-1.5 text-xs text-zinc-400">
              ...
              <ChevronRight size={15} className={openSection === 'rolagem' ? 'rotate-90' : ''} />
            </div>
          </div>

          {(openSection === 'rolagem' || isPlaying) && (
            <div className="mt-3 pt-2.5 border-t border-zinc-800 space-y-2.5">
              <div className="flex items-center justify-between">
                <button
                  onClick={onToggleScroll}
                  className={`px-3.5 py-2 rounded-xl font-bold text-xs sm:text-sm flex items-center gap-1.5 shadow-sm active:scale-95 transition-all ${
                    isPlaying
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-zinc-400">Vel:</span>
                  <input
                    type="range"
                    min="1"
                    max="10"
                    ...
                    className="w-20 accent-orange-500 h-1.5 bg-zinc-700 rounded-lg cursor-pointer"
                  />
                  <span className="font-mono text-orange-400 font-bold w-4 text-xs sm:text-sm">{speed}</span>
                </div>
              </div>
        ...
        <div
          ...
          className="p-3.5 sm:p-4 hover:bg-zinc-850/50 flex items-center justify-between cursor-pointer transition-colors"
        >
          <div className="flex items-center gap-2.5">
            <Columns size={17} className="text-zinc-400" />
        ...
        <div className="p-3.5 sm:p-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="w-7 h-7 rounded-xl bg-zinc-800 hover:bg-zinc-700 active:scale-95 flex items-center justify-center text-zinc-200 border border-zinc-700"
            >
              <Minus size={14} />
            </button>
            <span className="font-bold font-mono text-sm sm:text-base text-orange-500 min-w-[32px] text-center">
              {currentKey}
            </span>
            <button
              onClick={() => onTranspose(1)}
        ...
        >
          <div className="flex items-center gap-2.5">
            <BookOpen size={17} className="text-zinc-400" />
